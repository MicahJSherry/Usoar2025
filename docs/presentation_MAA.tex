...
\begin{frame}[shrink=7]{Necessary but Not Sufficient Condition for Factorization}
\small
\begin{theorem}
...
$\displaystyle f(x)=x^6+x^5+x^2+1$ does not satisfy the condition and  is not factorable.
\end{frame}
...
\begin{frame}{Examples of Possible Factorization }    
    ...
    \begin{frame}[shrink=5]{Factorizations Of The Form $(x+1)h(x)= f_n(x) (Cont.)$}
        ...
        \begin{theorem}
            Let $ G_{n \, 1}= \{h(x)|(x+1) h(x) = f_n(x) \}$
            Let $F_n$ be the $n\text{th}$ Fibonacci number.
            $|G_{n \, 1}| = F_{n}$ for $n \geq 1$ 
        \end{theorem}
        \begin{proof}[Proof (by Strong induction)]    
            \noindent $G_{1 \, 1} = \{1\}, \quad |G_{1 \, 1}| = 1 = F_1$
            \enspace \& \enspace
            $G_{2 \, 1} = \{x+1\}, \quad |G_{2 \, 1}| = 1 = F_2$
            
            Therefore the base cases hold. Assume that $|G_{j\,1}| = F_{j}$ for all $j \in \{1, 2, \ldots, k\}$ for some $k$. 
        
            Let $h(x) \in G_{(k+1)\, 1}$. Notice $\deg(h(x))=k$ and $h_k=1$ consider the possible values for $h_{k-1}$.
                \begin{enumerate}
                    \item Case 1. $h_{k-1} = 1$. In which case $h(x)=x^k + h'(x)$ Where $h'(x) \in G_{k\, 1}$.
                    \item Case 2. $h_{k-1} = 0$, So $h_{k-2} = 1$. In which case $h(x)=x^k + 0x^{k-1}+ h''(x)$ Where $h''(x) \in G_{(k-1) \, 1}$. 
                \end{enumerate}
                So, $|G_{(k+1) \, 1}| = |G_{k \, 1}| + |G_{(k-1)\, 1}| = F_{k} + F_{k-1} = F_{k+1}$.
                Therefore, by the Principle of Mathematical Induction $|G_{n\,1}| = F_{n}$ for all $n\ge1$.
        \end{proof}
    \end{frame}
    ...
    \begin{frame}{Factorizations Of The Form $(x^k+1)g(x)= f_n(x)$}  
        ...
        \begin{theorem}
            Let $G_{n \, k} = \{g(x)|(x^k+1) g(x)=f_n(x)\}$,
            let $m=n-3k+1$,
            let $q = \floor{m}{k}$,
            and let $r= m\mod k$.
            $$|G_{n \, k}| = (F_{q+3})^r \times (F_{q+2})^{k-r}$$
        \end{theorem}
        \begin{proof}[Sketch] 
